...
namespace BibliotecaEstudianteMain.Controllers
{
    class DBUtilsController
    {
        ...
        {
            SqliteQuery cql = new SqliteQuery();
            DataSet ds = cql.ExecQueryReturn("Select idTema as 'ID de Tema', NombreTema as 'Nombre del Tema', Descripcion, pagInicial as 'Inicio de Pagina', pagFinal as 'Fin de Pagina' from Temario where idLibro =" + idLibro);
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count <= 0)
                {
